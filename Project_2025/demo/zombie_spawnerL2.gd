...
var zombie_scene = preload("res://Assets/Zombies/ZombieTemplateLevel2.tscn")
...
func start_next_wave():
	wave_number += 1
	current_wave_zombie_count = min(wave_number * 5, 20)  # Increase difficulty but cap at 20
	zombies_killed_in_wave = 0

	print("🧟 Level 2: Starting wave ", wave_number, " with ", current_wave_zombie_count, " zombies.")
	spawn_zombies_in_wave(current_wave_zombie_count)

func spawn_zombies_in_wave(count):
	for i in range(count):
		await get_tree().create_timer(0.6).timeout  # Delay for natural wave look
		spawn_zombie()
...
func spawn_zombie():
	...
	spawn_pos += Vector2(randi_range(-4, 4), randi_range(-4, 4))  # Soft offset to avoid stacking

	var zombie = zombie_scene.instantiate()

	# Random animation type for visual variety
...
func _on_zombie_died():
	...
	if zombies_killed_in_wave >= current_wave_zombie_count:
		await get_tree().create_timer(2.0).timeout  # Wait before next wave
		start_next_wave()
